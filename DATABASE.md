...
The dataset behind [the parcel explorer](README.md) is a **public DuckDB
database** — every parcel's owner, appraised values, exemptions, zoning, legal
description, and geometry, updated daily, with a full **history trail**: you
can ask who owned any parcel, and what it was worth, at any point in time
since the archive started.

Anyone can query it over HTTPS with no credentials, no account, no API key, no
extension to install — just DuckDB (any version ≥ 1.0).
...
How the scrape, workspaces, audit, and publishing work — and how to publish
your own copy — is in [CONTRIBUTING.md](CONTRIBUTING.md).
