...
export const FiltersPanel: FC<{className?: string}> = ({className}) => {
  ...
  const isTableReady = useRoomStore((state) =>
    state.db.tables.find((t) => t.tableName === MAIN_TABLE),
  );
...
const FiltersPanelContent = ({className}: {className?: string}) => {
  ...
  // Get columns for the parcels table
  const columns: ChartBuilderColumn[] = useMemo(() => {
    const table = tables.find((t) => t.tableName === MAIN_TABLE);
  ...
  const handleCreateChart = useCallback(
    (title: string, config: ChartConfig) => {
      try {
        ...
        const spec = chartTypeDef.createSpec(MAIN_TABLE, config.settings);
